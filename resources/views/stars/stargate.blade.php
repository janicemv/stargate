            ...
            <table class="table-auto min-w-full text-sm text-left rtl:text-right">
                <thead class="text-xs text-gray-700 uppercase bg-purple text-white dark:bg-purple-medium">
                    <tr>
                        ...
                        <th scope="col" class="hidden lg:table-cell px-6 py-3">Designation</th>
                        <th scope="col" class="px-6 py-3">Constellation</th>
                        <th scope="col" class="hidden lg:table-cell px-6 py-3">HIP</th>
                        <th scope="col" class="hidden lg:table-cell px-6 py-3">Magnitude</th>
                        <th scope="col" class="hidden lg:table-cell px-6 py-3">Ascension</th>
                        <th scope="col" class="hidden lg:table-cell px-6 py-3">Declination</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($stars as $star)
                        <tr
                            ...
                            <td class="px-6 py-4"><a class="font-bold hover:underline"
                                    href="/star/{{ $star->id }}">{{ $star->name }}</a></td>
                            <td class="hidden lg:table-cell px-6 py-4">{{ $star->iau_desig }}</td>
                            <td class="px-6 py-4"><a href="/constellations/{{ $star->constellation->id }}"
                                    class="text-blue-500 hover:underline">
                                    {{ $star->constellation->name }}
                                </a></td>
                            <td class="hidden lg:table-cell px-6 py-4">{{ $star->iau_HIP }}</td>
                            <td class="hidden lg:table-cell px-6 py-4">{{ $star->Vmag }}</td>
                            <td class="hidden lg:table-cell px-6 py-4">{{ $star->RA_J2000 }}</td>
                            <td class="hidden lg:table-cell px-6 py-4">{{ $star->Dec_J2000 }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
